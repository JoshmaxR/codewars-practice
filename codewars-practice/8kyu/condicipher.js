...
function buildAlphabet(key) {
    return Array.from(new Set(key + "abcdefghijklmnopqrstuvwxyz"))
  }
  
  function encode(message, key, initShift) {
    const alphabet = buildAlphabet(key)
    let shift = initShift
    let result = ""
  ...
  function decode(message, key, initShift) {
    const alphabet = buildAlphabet(key)
    let shift = initShift - 1
    let result = ""
    for (const c of message) {
      let index = alphabet.indexOf(c)
      if (index >= 0) {
        shift = (51 + index - shift) % 26
