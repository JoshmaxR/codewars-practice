...
function canJump(arr, curr=0) {
    if (curr === arr.length - 1) return false;
    if (curr + arr[curr] >= arr.length) return true;
    for (let i = 1; i <= arr[curr]; i++) {
      if (canJump(arr, curr + i)) return true;
    }
    return false;
  }