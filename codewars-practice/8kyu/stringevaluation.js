...
function string_evaluation(str, conditions) {
    const count = char => Number(char) || str.split(char).length - 1
    const safe = ['<', '>', '>=', '<=', '==', '!=']
    return conditions
      .map(cond => [count(cond[0]), cond.slice(1, cond.length - 1), count(cond[cond.length - 1])])
      .map(cond => safe.includes(cond[1]) && eval(cond.join('')))
  }
